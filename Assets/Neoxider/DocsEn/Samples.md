...
After importing through Unity Package Manager, Unity copies the sample into the project-level path `Assets/Samples/NeoxiderTools/<version>/<sample name>/...`. For the current version the expected imported path is `Assets/Samples/NeoxiderTools/9.0.0/Demo Scenes/...`.
...
`SampleSceneCoverageTests` opens the required smoke scenes and verifies that they are not empty, contain `ModuleDemoSceneInfo`, and have no missing scripts. The RPG scene check also opens `RpgCombatNpcDemo` and `VampireSurvivorMCP`, verifying demo controllers, a player `RpgCharacter`, spawner wiring, and no missing scripts. It supports the development root, the UPM source root, and Unity's imported samples root.
