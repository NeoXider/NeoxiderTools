...
**Как использовать:**
1. Для нового кода не добавляйте `TimeReward` в сцену.
2. Используйте [CooldownReward](./CooldownReward.md) как актуальную замену.
3. Если в старой сцене уже стоит `TimeReward`, компонент можно оставить до плановой миграции.
4. Компонент скрыт из `Add Component` / `CreateFromMenu`, чтобы новые сцены не создавали legacy-зависимость случайно.

---

**Статус:** `deprecated`.

**Замена:** [CooldownReward](./CooldownReward.md).

**Пространство имен:** `Neo.Bonus`

**Путь к файлу:** `Assets/Neoxider/Scripts/Bonus/TimeReward/TimeReward.cs`

`TimeReward` — компонент таймера награды с сохранением времени в `SaveProvider`, расширенным публичным API управления жизненным циклом таймера и поддержкой накопления наград.
