...
**Улучшения в v5.2.21:**
- Оптимизирована производительность `NeoObjectPool`: добавлено кэширование компонентов `IPoolable`
- Компоненты кэшируются при первом обращении и очищаются при уничтожении объектов
- Это значительно снижает нагрузку от повторных вызовов `GetComponentsInChildren`
