...
CMF is bundled under the MIT license in `ThirdParty/CharacterMovementFundamentals/` (see the `LICENSE.md` there and the repository [THIRD-PARTY-NOTICES](../../../../THIRD-PARTY-NOTICES.md)). It is vendored as shipped, with one patch: `Mover.SetVelocity` writes `Rigidbody.linearVelocity`, the Unity 6 name for the old `velocity`. The patch is marked with a `NEOXIDER PATCH` comment. Two folders were renamed for path hygiene (`Animation & Audio` -> `AnimationAudio`, `Core scripts` -> `Core`); no script GUIDs changed.
