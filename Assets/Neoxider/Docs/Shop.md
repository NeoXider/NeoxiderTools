...
**Что это:** обзор модуля Shop (магазин, валюта, покупки). Навигация по классам — в [Shop/README](Shop/README.md).

**Как использовать:** см. разделы ниже или [Shop/README](Shop/README.md).
...
- **Три типа баланса**: `money` (текущий), `levelMoney` (за текущий уровень) и `allMoney` (все заработанные деньги).
...
**Описание**
`TextMoney` — это UI-компонент для отображения выбранного баланса из `Money`. Поддерживает режимы `Money`, `LevelMoney` и `AllMoney`, автоматически подписывается на нужное событие и обновляет текстовое поле без дополнительного кода.
