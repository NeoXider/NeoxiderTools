...
- **Мультиплеер**: Автоматически поддерживает **Mirror**. За счет встроенной проверки `hasAuthority` и наследования от `NetworkBehaviour` (если пакет установлен) компонент игнорирует пользовательский ввод для чужих персонажей, поэтому один префаб корректно работает как для локального, так и для сетевых игроков.
