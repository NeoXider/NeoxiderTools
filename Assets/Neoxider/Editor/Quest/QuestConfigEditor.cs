...
namespace Neo.Editor.Quest
{
    ...
    public sealed class QuestConfigEditor : CustomEditorBase
    {
        ...
        private void DrawSummary(SerializedProperty idProp, SerializedProperty titleProp, SerializedProperty iconProp,
            SerializedProperty objectivesProp, SerializedProperty startConditionsProp, SerializedProperty nextQuestIdsProp)
        {
            string title = string.IsNullOrWhiteSpace(titleProp.stringValue)
                ? target.name
                : titleProp.stringValue;
            string id = string.IsNullOrWhiteSpace(idProp.stringValue) ? "ID will be generated from title" : idProp.stringValue;

            List<NeoxiderEditorGUI.Badge> badges = new()
            {
                new(string.IsNullOrWhiteSpace(idProp.stringValue) ? "Draft" : "Configured",
                    string.IsNullOrWhiteSpace(idProp.stringValue)
                        ? new Color(0.76f, 0.48f, 0.16f, 1f)
                        : new Color(0.18f, 0.62f, 0.32f, 1f)),
                new($"Objectives {objectivesProp.arraySize}", new Color(0.20f, 0.50f, 0.78f, 1f)),
                new($"Start Conditions {startConditionsProp.arraySize}", new Color(0.42f, 0.34f, 0.82f, 1f)),
                new($"Next Quests {nextQuestIdsProp.arraySize}", new Color(0.24f, 0.60f, 0.60f, 1f)),
                new(iconProp.objectReferenceValue != null ? "Icon Ready" : "No Icon",
        ...
        private void DrawValidation(SerializedProperty idProp, SerializedProperty titleProp, SerializedProperty objectivesProp,
            SerializedProperty nextQuestIdsProp)
        {
            if (string.IsNullOrWhiteSpace(titleProp.stringValue))
            {
                EditorGUILayout.HelpBox("Заполни Title. Без него квест плохо читается в UI и ID не сможет автосгенерироваться.", MessageType.Warning);
            }
            ...
            {
                EditorGUILayout.HelpBox("В списке Next Quest Ids есть дубликаты. Лучше держать цепочку переходов уникальной.", MessageType.Warning);
            }

            if (string.IsNullOrWhiteSpace(idProp.stringValue) && !string.IsNullOrWhiteSpace(titleProp.stringValue))
            {
                EditorGUILayout.HelpBox("ID будет заполнен из Title при валидации asset или по кнопке Generate ID.", MessageType.Info);
            }
        }
