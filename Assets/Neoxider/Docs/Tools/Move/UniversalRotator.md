...
**Что это:** компонент плавного поворота к цели: 2D (по Z) или 3D (LookRotation), наведение на Transform/точку/мышь, ограничение по оси. Пространство имён `Neo.Tools`, файл `Scripts/Tools/Move/UniversalRotator.cs`.

**Как использовать:** добавить на объект, задать Rotation Mode и цель (Transform/Use Mouse World и т.д.); при необходимости ограничения угла и Update Mode. См. «Настройки инспектора» ниже. Поддерживает ограничение по одной оси, работу с мышью (2D/3D), поворот к `Transform`, мировой точке или направлению, а также выбор режима обновления и использование нескалированного времени.
