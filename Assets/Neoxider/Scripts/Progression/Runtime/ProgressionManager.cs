...
namespace Neo.Progression
{
    ...
    public sealed class ProgressionManager : Singleton<ProgressionManager>
    {
        ...
        [Header("Definitions")] [SerializeField] private LevelCurveDefinition _levelCurve;
        ...
        /// <summary>
        /// Gets the resolved current level.
        /// </summary>
        public int CurrentLevel => _profile.CurrentLevel;
        ...
        /// <summary>
        /// Adds XP and applies any resulting level rewards.
        /// </summary>
        ...
        public bool CanUnlockNode(string nodeId, out string failReason)
        {
            ...
            if (_profile.CurrentLevel < node.RequiredLevel)
            {
        ...
        public bool CanBuyPerk(string perkId, out string failReason)
        {
            ...
            if (_profile.CurrentLevel < perk.RequiredLevel)
            {
        ...
        private void ApplyProfile(ProgressionProfileData profile, bool invokeEvents)
        {
            ...
            ApplyDefaultStates();
            ImportProfileSets();
            _profile.CurrentLevel = ResolveLevelFromXp(_profile.TotalXp);
            SyncProfileListsFromSets();
        ...
        {
            int targetLevel = ResolveLevelFromXp(_profile.TotalXp);
            if (targetLevel <= _profile.CurrentLevel)
            {
                return;
            }

            for (int level = _profile.CurrentLevel + 1; level <= targetLevel; level++)
            {
        ...
        private int ResolveLevelFromXp(int totalXp)
        {
            return _levelCurve != null ? Mathf.Max(1, _levelCurve.EvaluateLevel(totalXp)) : Mathf.Max(1, _profile.CurrentLevel);
        }
        ...
        {
            XpState.SetValueWithoutNotify(_profile.TotalXp);
            LevelState.SetValueWithoutNotify(_profile.CurrentLevel);
            PerkPointsState.SetValueWithoutNotify(_profile.AvailablePerkPoints);
            XpToNextLevelState.SetValueWithoutNotify(_levelCurve != null ? _levelCurve.GetXpToNextLevel(_profile.TotalXp) : 0);
            ...
            _onXpChanged?.Invoke(_profile.TotalXp);
            _onLevelChanged?.Invoke(_profile.CurrentLevel);
