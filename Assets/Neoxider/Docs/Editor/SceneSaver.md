...
**Что это:** Утилита создает клон сцены в отдельной папке `Assets/Scenes/AutoSaves`, не затрагивая основной файл сцены. Это обеспечивает безопасное и ненавязчивое резервное копирование.
...
- **Путь к файлу**: `Assets/Neoxider/Editor/Scene/SceneSaver.cs`
- **GUI класс**: `SceneSaverGUI` (`Assets/Neoxider/Editor/GUI/SceneSaverGUI.cs`)
- **Доступ в меню**: `Tools/Neoxider/Scene Saver`

**Описание**
Редакторный скрипт, который автоматически сохраняет резервные копии активной сцены. Использует архитектуру с разделением логики и GUI отрисовки.
...
- **Профессиональная архитектура**: GUI отрисовка вынесена в отдельный класс `SceneSaverGUI`.
