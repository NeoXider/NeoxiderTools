...
namespace Neo.Editor
{
    ...
    public abstract class CustomEditorBase : UnityEditor.Editor
    {
        ...
        protected Dictionary<string, object> _buttonParameterValues = new Dictionary<string, object>();
        protected Dictionary<string, bool> _buttonFoldouts = new Dictionary<string, bool>();
        protected Dictionary<string, bool> _isFirstRun = new Dictionary<string, bool>();
        ...
        protected static T[] FindObjectsByType<T>(FindObjectsSortMode sortMode = FindObjectsSortMode.None) where T : UnityEngine.Object
        {
        ...
        protected static UnityEngine.Object[] FindObjectsByType(Type type, FindObjectsSortMode sortMode = FindObjectsSortMode.None)
        {
        ...
        protected virtual void DrawMethodButtons()
        {
            ...
            foreach (var method in methods)
            {
                ...
                if (GUILayout.Button(buttonText, EditorStyles.miniButton, GUILayout.Width(120)))
                {
                    try
                    {
                        ...
                        for (int i = 0; i < parameters.Length; i++)
                        {
                            ...
                            {
                                Debug.LogWarning($"Parameter '{param.Name}' of method '{method.Name}' is null. Using default value.");
                                value = GetDefaultValue(param.ParameterType);
                                _buttonParameterValues[key] = value;
                            }
                    ...
                    {
                        Debug.LogError($"Error calling method {method.Name}: {e.InnerException?.Message ?? e.Message}\nStack trace: {e.InnerException?.StackTrace ?? e.StackTrace}");
                    }
                }
                ...
                if (parameters.Length > 0)
                {
                    ...
                    _buttonFoldouts[foldoutKey] = EditorGUILayout.Foldout(_buttonFoldouts[foldoutKey], "Parameters", true);
        ...
        protected object DrawParameterField(string label, object value, Type type)
        {
            try
            {
                ...
                {
                    result = EditorGUILayout.Vector2Field(label, value is Vector2 vector2Value ? vector2Value : Vector2.zero);
                }
                else if (type == typeof(Vector3))
                {
                    result = EditorGUILayout.Vector3Field(label, value is Vector3 vector3Value ? vector3Value : Vector3.zero);
                }
