...
**Что это:** синглтон воспроизведения звуков и музыки. Два канала (эффекты и музыка), массивы клипов, Play(id)/Play(AudioClip). Пространство имён: `Neo.Audio`. Файл: `Scripts/Audio/AudioSimple/AM.cs`.

**Как использовать:** в сцене один объект с AM; из кода вызывать AM.I.Play(id) или AM.I.Play(clip); для музыки — методы музыки; опционально RandomMusicController.
