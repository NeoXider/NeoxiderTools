...
**Улучшения в v5.2.21:**
- Улучшена обработка камеры: добавлен fallback на `FindFirstObjectByType<Camera>()` при отсутствии `Camera.main`
- Компонент автоматически отключается при отсутствии камеры с информативным сообщением об ошибке
- Это предотвращает `NullReferenceException` и улучшает отладку
