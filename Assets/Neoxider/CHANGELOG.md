...
- **Character prefabs are drop-in now**: the two capsule prefabs got footstep/jump/land audio (`AudioControl`, CMF clips vendored into `Audio/Character Controller/` — no sample dependency) and mouse-look prefabs (First/Third Person, Third Person/Top Down Animated) carry a `CursorLockController` (lock on start, Escape toggles), so look works in an empty scene without extra wiring. All character cameras clear to solid black instead of the skybox.
