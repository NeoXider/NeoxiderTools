...
**Что это:** MonoBehaviour-компонент таймера на GameObject (пространство имён `Neo`, файл `Scripts/Tools/Time/TimerObject.cs`). Обратный/прямой отсчёт, зацикливание, игнор Time.timeScale, UnityEvent на старте/обновлении/завершении. Подходит для кулдаунов, обратных отсчётов, длительностей эффектов.

**Как использовать:** добавить на GameObject, задать duration и режим в инспекторе; подписаться на OnTimeChanged, OnTimerComplete и др. или использовать Auto Actions. См. секции ниже.
...
- Добавлена секция "Auto Actions" для автоматических действий в инспекторе
...
- `timeScale` (по умолчанию `1`): Множитель скорости времени. Значение `2` — время идёт в 2 раза быстрее. Применяется к `deltaTime` при обновлении. Удобно для «игровых суток» или ускоренного таймера.
- `looping`: Если `true`, таймер будет автоматически перезапускаться после завершения.

### Day Time (время суток 00:00–24:00)

- `dayLengthSeconds` (по умолчанию `0` = выключено): При значении `86400` (24 часа в секундах) таймер интерпретируется как цикл «день»: время зацикливается в диапазоне `[0, dayLengthSeconds)`, доступны свойства времени суток и события дня.
- `_dayTimeFormat`: Строка формата для вывода времени суток в `timeText` при включённом дне. Плейсхолдеры: `{0}` = часы, `{1}` = минуты, `{2}` = секунды (например `"{0:00}:{1:00}"` для HH:mm).
- **Свойства только для чтения:** `DayTimeHours` (0–24), `DayTimeMinutes` (0–59), `DayTimeSeconds` (0–59) — вычисляются из текущего времени при `dayLengthSeconds > 0`.
- **События:** `OnNewDay` — вызывается при переходе через полночь (когда при `dayLengthSeconds > 0` и `looping` время перескакивает с конца дня на 0). `OnDayTimeChanged (float hours, float minutes, float seconds)` — при каждом обновлении времени суток (по `updateInterval`).
...
- `OnNewDay`: Вызывается при переходе через полночь (когда при `dayLengthSeconds > 0` и `looping` значение перескакивает с конца дня на 0).
- `OnDayTimeChanged (float, float, float)`: Передаёт часы, минуты, секунды времени суток при каждом обновлении (только при `dayLengthSeconds > 0`).
