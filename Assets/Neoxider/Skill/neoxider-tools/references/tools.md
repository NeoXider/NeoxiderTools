...
- **3D character controller (v10.3+, preferred)** — CMF motor (`ThirdParty/CharacterMovementFundamentals`: `Mover` + `AdvancedWalkerController`, slope limit with slide-off, stairs, moving platforms via `CMF.MovingPlatform`+`TriggerArea`, momentum `AddMomentum/SetMomentum`, `OnJump/OnLand` C# events) wrapped by Neo components in `Scripts/Tools/Move/CharacterController/`: **`NeoCharacterInput`** (New/legacy input auto-detect; injection `SetMoveInput(Vector2?)/SetJumpInput(bool held)/SetRunInput(bool)`; gates `SetMovementEnabled/SetJumpEnabled`), **`NeoCameraInput`** (look; `GameSettings.MouseSensitivity`, cursor gate, EM pause; `SetLookInput(v?)`), **`NeoCharacterSprint`** (`IsSprinting`, multiplier over `movementSpeed`), **`NeoCharacterCameraBridge`** (Cinemachine-driven camera), **`NeoCharacterNetworkBinding`** (Mirror local/remote split). Ready prefabs in Create window *Presets → Player*: Character First/Third Person (capsule, self-contained, drop-in: footstep audio + `CursorLockController` on root) + Animated/Top Down/Side Scroller/Click To Move variants (need the CMF sample imported for Capguy assets); *Presets → Environment → Moving Platform*. Docs: `Docs/Tools/Move/CharacterController/`.
- **`PlayerController2DPhysics` / `PlayerController3DPhysics`** (3D one is legacy since v10.3, Mirror-aware) — `SetMovementEnabled/SetJumpEnabled/SetLookEnabled`, `Teleport(pos)`, external input `SetMoveInput(v?)/SetJumpInput()/SetRunInput(bool)`; props `IsGrounded`, `IsRunning`. `SetJumpInput()` is also a Play Mode-only **Jump** Inspector button. Animator drivers `PlayerController2D/3DAnimatorDriver` **[no-code]** observe them.
...
## Interaction / Physics
- **`InteractiveObject`** (Mirror-aware) — hover/click/range; `interactable`, `InteractionDistance`, `IsHovered`, `IsInInteractionRange`, `UseScreenCenterRay`; typed manual dispatch through `InteractDown()`, `InteractUp()`, `Click(MouseButton,bool)`; events `onInteractDown/Up`, `onHoverEnter/Exit`, `onClick/onDoubleClick/onRightClick`, `onEnterRange/onExitRange`. Depend on **`IInteractiveTarget`** for custom AI/XR/proximity input. Reuse allocation-free **`InteractionQueryMath`** / **`InteractionRayHit`** for range and ordered-hit rules, and **`InteractionCameraResolver`** for the shared cached-camera policy. These reusable APIs stay in namespace `Neo.Tools` but live in the dependency-free `Neo.Tools.InteractableObject.Core` assembly; custom asmdefs should reference that leaf directly when they do not need the scene component or Mirror integration. The manual dispatch methods and collider-cache invalidation have Play Mode-only Inspector buttons for scene testing.
...
- **[no-code] view bits**: `TextScore` (auto-binds ScoreManager), `StarView` (binds CountStars), `LightAnimator` (`Play/Stop/Pause/Resume`, also Play Mode-only Inspector buttons), `MeshEmission`, `DOTweenUIImageFallback`, `UpdateChilds`.
...
- **`TypewriterEffect`** (`[Serializable]`, UniTask) — `PlayAsync(text,onTextChanged,ct)`, `Stop()`, `Complete()`; `IsTyping`, `Progress`; events `OnComplete`, `OnCharacterTyped(char)`. Scene wrapper **`TypewriterEffectComponent`** exposes `PlayAutoText()`, `Stop()`, and `Clear()` as Play Mode-only Inspector buttons.
