...
- **CMF showcase controllers as presets**, rewired from CMF's legacy-Input scripts to `NeoCharacterInput`/`NeoCameraInput`/`NeoCharacterSprint`: `Character First Person (Audio)`, `Character Third Person (Animated)`, `Character Top Down (Animated)`, `Character Side Scroller (Animated)`, `Character Click To Move (Animated)` (keeps its mouse-raycast input) in *Presets → Player*, and `Moving Platform` in a new *Presets → Environment* category. The animated ones reference Capguy/audio assets from the imported CMF sample — see the CharacterController README.
