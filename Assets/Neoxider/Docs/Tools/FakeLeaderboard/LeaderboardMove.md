...
**Что это:** Дополнительно он может проигрывать анимацию на элементе игрока, чтобы привлечь к нему внимание. Это создает приятный и сфокусированный пользовательский опыт, избавляя игрока от необходимости вручну...
...
**Ключевые поля**
- `leaderboard`: (Опционально) Конкретный лидерборд для использования. Если не указан, используется синглтон `Leaderboard.I`. Позволяет работать с несколькими лидербордами на сцене.
...
- `scaleDelta`: Насколько будет увеличен элемент игрока во время анимации (дельта к текущему размеру, по умолчанию `0.1f`).
- `durationAnimPlayer`: Длительность анимации выделения.
- `useAnimStartScale`: Возвращать ли элемент к исходному размеру после анимации выделения.
- `useSortEnable`: Автоматически сортировать лидерборд при включении компонента.

**Поведение масштабирования**
- При каждом вызове `Move()` компонент один раз запоминает текущий размер карточки игрока перед запуском анимации.
- Предыдущая выделенная карточка (если есть и это уже не игрок) возвращается к своему исходному размеру.
- Текущая карточка игрока временно увеличивается на дельту `+scaleDelta` к своему базовому размеру.
- Если `useAnimStartScale == true`, после анимации элемент игрока плавно возвращается к своему исходному масштабу.
...
- **Leaderboard**: Компонент обращается к указанному лидерборду (или к синглтону `Leaderboard.I`, если лидерборд не указан). Убедитесь, что `Leaderboard` присутствует на сцене.

**Работа с несколькими лидербордами:**
Если на сцене несколько лидербордов, вы можете указать конкретный лидерборд в поле `leaderboard`. Если поле оставить пустым, будет использоваться синглтон `Leaderboard.I`.
