...
- `checkObstacles` (по умолчанию `true`): Проверка препятствий (стен) между объектом и точкой проверки
  ...
  - Если выключить, препятствия игнорируются и для дистанции, и для keyboard look-ray (`requireDirectLookRay`)
...
- если `interactionDistance > 0`, наведение требует попадания в collider **и** нахождения объекта в радиусе взаимодействия.
