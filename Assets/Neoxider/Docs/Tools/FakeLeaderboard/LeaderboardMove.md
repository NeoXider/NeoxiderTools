...
- При каждом вызове `Move()` все элементы лидерборда сначала возвращаются к базовому масштабу.
- Элемент игрока временно увеличивается на дельту `+0.1f` к базовому размеру.
- Если `useAnimStartScale == true`, после анимации элемент игрока плавно возвращается к базовому масштабу.
