...
### Основные компоненты
- [**`SpinController`**](./SpinController.md): Главный класс, управляющий логикой спинов, ставок и результатами; опциональная анимация выигрышных линий через **LineRenderer** (`WinLineRendererPlayback`, см. страницу). Из кода: матрицы линий, **`SlotElement`** для анимаций, **`ConfigureSlotRuntime`**, снимок состояния **`GetRuntimeSnapshot`**.
