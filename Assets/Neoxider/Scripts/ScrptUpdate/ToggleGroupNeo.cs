...
{
    [AddComponentMenu("Neoxider/UI/Toggle Group", 31)]
    ...
    /// </remarks>
    public class ToggleGroupNeo : UIBehaviour
    {
        ...
        public UnityEvent<int> OnSetToggle;

        protected ToggleGroupNeo()
        { }
        ...
        public void EnsureValidState()
        {
            ...
            List<Toggle> activeToggles = ActiveToggles().ToList();
            ...
            if (activeToggles.Count > 1)
            {
                ...
                if (firstActive != null)
                {
                    int index = m_Toggles.IndexOf(firstActive);

                    OnSetToggle?.Invoke(index);

                    for (int i = 0; i < activeToggles.Count; i++)
                    {
                        Toggle toggle = activeToggles[i];
