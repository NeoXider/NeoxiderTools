...
**Что это:** Основная задача этого компонента — получить данные о пользователе (`LeaderboardUser`) от главного менеджера `Leaderboard` и распределить эту информацию по соответствующим текстовым полям (ранг, имя...
...
**Ключевые поля**
- `textScore` (`TMP_Text`): Ссылка на текстовый компонент для отображения очков. Счет автоматически форматируется согласно настройкам `Leaderboard` (форматирование чисел или времени).
...
- `user`: Текущий пользователь, данные которого отображаются в этом элементе.
- `id`: Номер позиции пользователя в списке.
- `isPlayer`: Является ли этот элемент записью реального игрока.

**Публичные методы (Public Methods)**
- `Set(LeaderboardUser user, bool isPlayer, Leaderboard leaderboard = null)`: **Основной метод компонента.** Он вызывается `Leaderboard`-ом при создании или обновлении списка. Он принимает данные пользователя, заполняет текстовые поля с учетом форматирования из `Leaderboard` и вызывает события в зависимости от того, является ли эта строка записью реального игрока.
  - `user`: Данные пользователя для отображения.
  - `isPlayer`: Является ли этот пользователь реальным игроком.
  - `leaderboard`: (Опционально) Ссылка на лидерборд для получения настроек форматирования. Если не указана, счет отображается без форматирования.
