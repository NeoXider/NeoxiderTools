...
Вы не используете этот файл напрямую. Вместо этого вы создаете свои собственные компоненты, которые реализуют этот интерфейс. Например, `KeyboardMover` и `MouseMover2D` из этого же набора инструментов реализуют `IMover`.
