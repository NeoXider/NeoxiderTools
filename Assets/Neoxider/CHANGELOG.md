...
- **RPG input/save quality** — `RpgAttackController` now supports built-in configurable input with LMB primary attack by default, `RpgEvadeController` supports configurable built-in evade input, and `RpgStatsManager` now has optional `Auto Save` (disabled by default) plus explicit provider flush on manual save.
- **RPG targeting/presets** — added `RpgTargetSelector` and `RpgAttackPreset` for AI, skills, and spells; `RpgAttackController` can now use presets, `RpgNoCodeAction` can trigger preset execution, and inspector testing coverage was improved with extra `UnityEvent` hooks and `[Button]` methods on key RPG `MonoBehaviour` scripts.
- **NPC + RPG integration** — added `NpcCombatPreset`, `NpcRpgCombatBrain`, and `NpcCombatDecisionCore` so combat NPCs can be assembled from small components (`NpcNavigation` + `RpgTargetSelector` + `RpgAttackController` + `RpgCombatant`) instead of a single large custom script; the brain supports melee and ranged enemies through preset-driven chase/hold/attack flow and restores the previous navigation mode when combat ends.
...
- **RPG/NPC tests** — added edit mode tests for combatant invulnerability, direct attacks, preset forced-target execution, NPC combat decision rules, evade invulnerability, plus previous damage/heal/save-load coverage.
...
- **Docs** — main README, Docs/README, NPC docs, and AttackSystem README updated to feature RPG combat/NPC composition and deprecate legacy combat components.
...
- **TextLevel** — инициализация в Start (не OnEnable), опциональное поле **Level Source** (при нескольких LevelManager в сцене).
- **TextScore** — инициализация в Start, опциональное поле **Score Source**; в OnDisable отписка от сохранённого экземпляра (_scoreManager), а не от ScoreManager.I.

## [7.3.0] - 2026-02-23

### Shop / Money

- **Singleton<T>** — при обращении к `I` до Awake экземпляр больше не выбирается «первым найденным»: ищутся все объекты типа T, синглтоном назначается **первый, у кого включён Set Instance On Awake**. Если ни у одного флаг не включён — `_instance` не выставляется (будет назначен в Awake того, у кого флаг включён). Это устраняет ситуацию, когда в сцене два Money (монеты и энергия) и в билде синглтоном становилась энергия, а объект с монетами удалялся.
- **TextMoney** — инициализация перенесена в **Start** (не OnEnable), чтобы все синглтоны успели отработать Awake. Добавлено опциональное поле **Money Source**: если задано, отображается значение из указанного Money (например энергия); иначе используется `Money.I`. При повторном OnEnable подписка восстанавливается.

### Документация

- Обновлены Singleton.md (логика выбора по Set Instance On Awake), TextMoney.md (Money Source, Start), Money.md (несколько экземпляров, флаг синглтона).
