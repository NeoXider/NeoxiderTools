...
## Типизация: явные типы вместо var

- Не использовать `var`: локальные переменные, поля и возвращаемые значения объявлять с явным типом.
- Исключение — только анонимные типы (`var x = new { ... }`), где явный тип невозможен.
- Это часть контракта библиотеки: код должен быть читаемым без подсказок IDE и стабильным при рефакторинге.
...
Все `.md`, `.cs`, `.json`, `.asmdef`, `.txt` и похожие текстовые файлы должны оставаться нормальным UTF-8. Русский и английский текст нужно писать реальными символами, а не mojibake-последовательностями. Даже в документации и правилах не вставляйте такие последовательности буквальным текстом; если нужно привести пример, описывайте его через Unicode-коды или словами.

Важный урок: PowerShell/консоль иногда отображает корректный UTF-8 как странные символы. Нельзя чинить или переписывать русские документы только по визуальному выводу `Get-Content`, `git diff` или консоли. Перед исправлением нужно проверить реальное содержимое файла через UTF-8 чтение, например Python `Path(...).read_text(encoding="utf-8")`, `unicode_escape`, тест `PackageTextFiles_DoNotContainKnownMojibakeMarkers` или проектный `scripts/fix_mojibake.py`.

Если текст действительно сломан, исправлять нужно аккуратно и точечно, сохраняя нормальный UTF-8. Если проверка показывает, что файл корректный, а странные символы видны только в консоли, файл трогать нельзя.
...
После импорта UPM sample через Unity Package Manager содержимое `Samples~` копируется Unity не обратно в пакет, а в проектный путь вида `Assets/Samples/NeoxiderTools/<version>/<sample name>/...`. Для текущей версии это, например, `Assets/Samples/NeoxiderTools/10.13.7/Demo Scenes/...`.
