...
3. При подключении игрока сервер создаёт копию, очищает скопированный `sceneId`, назначает стабильный runtime `assetId` и вызывает `NetworkServer.AddPlayerForConnection`.
