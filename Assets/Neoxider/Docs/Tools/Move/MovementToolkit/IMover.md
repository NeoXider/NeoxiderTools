...
**Что это:** В данном случае, любой компонент, реализующий `IMover`, гарантирует, что у него будет способ сообщить, движется ли он (`IsMoving`), и два стандартных метода для управления его движением (`MoveDelta...
...
Вы не используете этот файл напрямую. Вместо этого вы создаете свои собственные компоненты, которые реализуют этот интерфейс. Реализуют `IMover`: `KeyboardMover`, `MouseMover2D` и `MouseMover3D` из этого же набора инструментов.
