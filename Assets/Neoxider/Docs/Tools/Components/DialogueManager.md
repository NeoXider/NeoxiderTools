...
**Улучшения в v5.2.21:**
- Оптимизирован Typewriter эффект: заменен `StringBuilder.ToString()` на `Substring()` для уменьшения аллокаций памяти
- Это значительно снижает нагрузку на GC при отображении длинных текстов
...
### 3.3. Поведение
- `autoStart` (`bool`): Если `true` (по умолчанию), первый диалог запустится автоматически при `Start()`.
- `autoNextSentence` (`bool`): Если `true`, следующая реплика начнется автоматически после завершения предыдущей.
- `autoNextMonolog` (`bool`): Если `true`, следующий монолог начнется автоматически после завершения предыдущего.
- `autoNextDialogue` (`bool`): Если `true`, следующий диалог начнется автоматически после завершения предыдущего.
- `allowRestart` (`bool`): Если `true`, разрешает перезапуск текущего диалога.
