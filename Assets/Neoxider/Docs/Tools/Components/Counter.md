...
2. Выберите режим **Int** или **Float**, задайте начальное значение.
3. Подпишите **OnValueChangedInt** (или OnValueChangedFloat) на компонент вывода текста (например [SetText](../Text/SetText.md)) или свой метод — значение будет обновляться при любом изменении.
...
**Описание**:
Универсальный счётчик: хранит число (по умолчанию 1), методы Add/Subtract/Multiply/Divide/Set, Send с событием. События по типу числа: в режиме Int вызываются OnValueChangedInt и OnSendInt, в режиме Float — OnValueChangedFloat и OnSendFloat. Опционально сохраняет значение через SaveProvider по ключу (по умолчанию выключено).
...
- **События по типу**: в режиме Int вызываются `OnValueChangedInt`/`OnSendInt`, в Float — `OnValueChangedFloat`/`OnSendFloat`. Дополнительно всегда вызываются `OnValueChanged`/`OnSend` (float).
...
- **Вызов событий при загрузке**: опция **Invoke Events On Load** (по умолчанию **вкл.**). Если включена, после загрузки значения в Start вызываются те же события, что и при изменении (OnValueChanged, OnValueChangedInt / OnValueChangedFloat), чтобы UI и подписчики сразу отобразили загруженное значение. Без этой опции загрузка произойдёт, но подписанный текст или логика не обновятся до первого изменения счётчика.
...
- `_value` (`float`): Текущее значение счётчика (по умолчанию 1).
...
- **Сохранение**
  ...
  - `_invokeEventsOnLoad` (`bool`): При загрузке в Start вызывать OnValueChanged* (чтобы UI применил загруженное значение). По умолчанию **вкл.** Отключите, если не хотите уведомлять подписчиков при старте.
- **События по типу**
...
- **События общие**
  - `OnValueChanged` (`UnityEvent<float>`): Вызывается при любом изменении (float).
...
- **Общие**  
  - `OnValueChanged` (`UnityEvent<float>`): при любом изменении значения.  
...
- **При старте**: значение загружается через `SaveProvider.GetFloat(_saveKey, _value)`. Если включено **Invoke Events On Load** (по умолчанию да), после загрузки вызываются события OnValueChanged / OnValueChangedInt / OnValueChangedFloat — подписчики (например SetText) сразу отображают загруженное значение.
- **При изменении значения**: сохраняется через `SaveProvider.SetFloat(_saveKey, _value)` и `SaveProvider.Save()`.
