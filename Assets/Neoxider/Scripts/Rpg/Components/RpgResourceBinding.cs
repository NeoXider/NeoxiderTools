...
namespace Neo.Rpg.Components
{
    ...
    {
        [Tooltip("Character whose resource is bound. When empty, searches up the hierarchy.")]
        [SerializeField] private RpgCharacter _character;

        [Tooltip("Resource id (preset or Custom string).")]
        [SerializeField] private RpgStatId _resourceId = new(RpgStatPreset.Hp);

        [Header("Events (per-frame, drives UI)")]
        [SerializeField] private UnityEventFloat _onCurrent = new();
        ...
        public RpgStatId ResourceId { get => _resourceId; set { Unsubscribe(); _resourceId = value; SubscribeIfReady(); } }
        ...
        private void SubscribeIfReady()
        {
            ...
            if (current != null) current.AddListener(HandleCurrent);
            if (max != null) max.AddListener(HandleMax);
            if (percent != null) percent.AddListener(HandlePercent);

            // Push initial values so UI is correct on first frame.
            if (current != null) _onCurrent?.Invoke(current.CurrentValue);
            if (max != null) _onMax?.Invoke(max.CurrentValue);
            if (percent != null) _onPercent?.Invoke(percent.CurrentValue);
        ...
        private void HandleCurrent(float v) => _onCurrent?.Invoke(v);
        private void HandleMax(float v) => _onMax?.Invoke(v);
        private void HandlePercent(float v) => _onPercent?.Invoke(v);
    }
}
