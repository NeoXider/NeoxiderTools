...
**Что это:** Этот компонент идеален для быстрой настройки управления в прототипах, для создания отладочных клавиш (например, "нажми G, чтобы получить золото") или для обработки простых игровых действий.
...
**Примечание**: Событие срабатывает один раз при нажатии (down). Работает со старой и новой системой ввода (KeyInputCompat). Раньше использовался `Input.GetKeyDown()`, поэтому событие срабатырует только один раз в момент нажатия клавиши, а не каждый кадр, пока она зажата.
