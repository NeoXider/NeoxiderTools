...
**Что это:** Готовый компонент `DrunkardGame` для создания классической карточной игры без написания кода.
...
**BoardComponent Reset (RestoreAllSourcesToBoard):**
- В `BoardComponent` появились сериализуемые списки:
  - **Hand Sources** — руки, из которых при рестарте собираются карты
  - **Board Sources** — другие доски, которые нужно очистить в текущую
  - **Extra Roots** — дополнительные корни, из которых соберутся CardComponent (активные и неактивные)
- **Initial Spawned Cards** — список всех карт, созданных на Initial Board при старте (виден в инспекторе).
- Кнопка/метод `RestoreAllSourcesToBoard()` собирает все карты из указанных источников и возвращает их на эту доску (при необходимости вместимость доски расширяется автоматически).
